...
except ModuleNotFoundError:  # LIBERO remains importable as an optional feature.
    gym = None
    spaces = None
import numpy as np
import torch
...
except ModuleNotFoundError:  # Defer the heavy simulator dependency until construction.
    benchmark = None
...
def _require_libero_dependencies() -> None:
    missing = []
    if gym is None:
        missing.append("gymnasium")
    if benchmark is None or OffScreenRenderEnv is None or get_libero_path is None:
        missing.append("libero (with robosuite/MuJoCo)")
    if missing:
        raise ModuleNotFoundError(
            "LIBERO support requires optional dependencies: " + ", ".join(missing)
        )


def _get_suite(name: str):
    """Instantiate a LIBERO suite by name with clear validation."""
    _require_libero_dependencies()
...
class LiberoEnv(_GymEnvBase):
    ...
    def reset(self, seed=None, **kwargs):
        ...
        if self.init_states and self._init_states is not None:
            # Preserve the 5090 training/evaluation protocol: LIBERO's saved
            # simulator state is selected before reset, then reset applies it.
            self._env.set_init_state(self._init_states[self.init_state_id % len(self._init_states)])
