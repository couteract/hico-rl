...
class ValueInferenceACPConfig:
    ...
    # Optional fixed normalization contract for episode-subset/OOF inference.
    # Keys are PyTorch LIBERO task_index values and values are frame lengths.
    task_max_lengths: dict[int, int] | None = None
    # When evaluating a held-out fold, persist its identity next to every
    # prediction. This lets the final ACP pass prove that every frame is OOF.
    oof_fold_index: int | None = None
    oof_fold_field: str = "complementary_info.value_oof_fold"
    ...
    def validate(self) -> None:
        ...
        if self.task_max_lengths is not None:
            self.task_max_lengths = {int(k): int(v) for k, v in self.task_max_lengths.items()}
            if not self.task_max_lengths or any(v <= 0 for v in self.task_max_lengths.values()):
                raise ValueError("'acp.task_max_lengths' must contain positive lengths.")
        if self.oof_fold_index is not None:
            if self.oof_fold_index < 0:
                raise ValueError("'acp.oof_fold_index' must be non-negative.")
            if not self.oof_fold_field:
                raise ValueError("'acp.oof_fold_field' must be non-empty when writing OOF predictions.")
