...
# This checkout can be a reduced benchmark distribution. Keep optional policy
# configs lazy so the SmolVLA/SAC path does not depend on omitted modules.
try:
...
def make_pre_post_processors(
    ...
    **kwargs: Unpack[ProcessorConfigKwargs],
    ...
    if pretrained_path:
        ...
        if GrootConfig is not None and isinstance(policy_cfg, GrootConfig):
    ...
    if TDMPCConfig is not None and isinstance(policy_cfg, TDMPCConfig):
    ...
    elif DiffusionConfig is not None and isinstance(policy_cfg, DiffusionConfig):
    ...
    elif ACTConfig is not None and isinstance(policy_cfg, ACTConfig):
    ...
    elif VQBeTConfig is not None and isinstance(policy_cfg, VQBeTConfig):
    ...
    elif PI0Config is not None and isinstance(policy_cfg, PI0Config):
    ...
    elif PI05Config is not None and isinstance(policy_cfg, PI05Config):
    ...
    elif Evo1Config is not None and isinstance(policy_cfg, Evo1Config):
    ...
    elif GrootConfig is not None and isinstance(policy_cfg, GrootConfig):
