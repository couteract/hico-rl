...
def run_value_inference_pipeline(
    ...
    if accelerator.is_main_process:
        ...
        if cfg.acp.oof_fold_index is not None:
            columns[cfg.acp.oof_fold_field] = np.full(
                predicted_values.shape, cfg.acp.oof_fold_index, dtype=np.int64
            )
            feature_infos[cfg.acp.oof_fold_field] = {
                "dtype": "int64",
                "shape": (1,),
                "names": None,
            }
        ...
        if cfg.acp.enable:
            ...
            if cfg.acp.task_max_lengths is not None:
                configured = {int(k): int(v) for k, v in cfg.acp.task_max_lengths.items()}
                referenced = set(task_indices.tolist())
                if set(configured) != referenced:
                    raise ValueError(
                        "acp.task_max_lengths must cover exactly the task indices in the loaded dataset: "
                        f"expected={sorted(referenced)} actual={sorted(configured)}"
                    )
                observed = set(task_max_lengths)
                if any(configured[k] < task_max_lengths[k] for k in observed):
                    raise ValueError(
                        "acp.task_max_lengths cannot be shorter than an observed episode in the loaded subset."
                    )
                task_max_lengths = configured
        ...
        result = {
            ...
            "thresholds": thresholds,
            "oof_fold_index": cfg.acp.oof_fold_index,
            "oof_fold_field": cfg.acp.oof_fold_field if cfg.acp.oof_fold_index is not None else None,
            "viz_outputs": viz_outputs,
        }
    else:
        result = {
