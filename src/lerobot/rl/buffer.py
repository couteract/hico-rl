...
from lerobot.utils.constants import ACTION, DONE, OBS_IMAGE, REWARD, TRUNCATED
...
class ReplayBuffer:
    ...
    def to_lerobot_dataset(
        ...
        # Add transition boundary fields. Keep truncation separate from done so
        # time-limit episodes survive ReplayBuffer -> LeRobotDataset round trips.
        features[REWARD] = {"dtype": "float32", "shape": (1,)}
        features[DONE] = {"dtype": "bool", "shape": (1,)}
        features[TRUNCATED] = {"dtype": "bool", "shape": (1,)}
        ...
        for idx in range(self.size):
            ...
            frame_dict[TRUNCATED] = torch.tensor(
                [self.truncateds[actual_idx]], dtype=torch.bool
            ).cpu()
    ...
    def _lerobotdataset_to_transitions(
        ...
        has_truncated_key = TRUNCATED in sample
        ...
        for i in tqdm(range(num_frames)):
            ...
            # Preserve a real time-limit boundary when the dataset carries it.
            # Older datasets without the field remain compatible and default to
            # false; `done` still closes their inferred episode boundary.
            truncated = bool(current_sample[TRUNCATED].item()) if has_truncated_key else False
            if truncated and not done:
                raise ValueError("A truncated dataset transition must also have next.done=true.")
